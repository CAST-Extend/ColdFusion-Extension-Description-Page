...
- corrects an issue with ReferenceFinder not accepting ^ as starting lines

What's new in 1.4.6 :

- corrects an issue with search_objects(..., load_properties=True)
...
if StrictVersion(get_version(cast_module)) < StrictVersion('1.4.7'):
    """
    lower version so we install ourselves
    """
    setattr(cast_module, '__version__', '1.4.7')
